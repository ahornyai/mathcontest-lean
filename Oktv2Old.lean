...
theorem sq_mul_x_is_sq {a x b : ℕ} (ha : a ≠ 0) (hb : b ≠ 0) (h : a*x = b) (ha_sq : IsSquare a) (hb_sq : IsSquare b) :
  IsSquare x := by {
  -- Először megállapítjuk, hogy a*x négyzetszám
  have h_ax_square : IsSquare (a * x) := by {
    rw [h]
    exact hb_sq
  }
  ...
  -- állapítsuk meg, hogy x négyzetszám
  -- a = c^2 és b = d^2
  have h1 : c^2 * x = d^2 := by linarith

  -- Most megmutatjuk, hogy d osztható c-vel
  have h_div_triv : c^2 ∣ d^2 := by exact Dvd.intro x h1

  have h_div : c ∣ d := by {
    rw [Nat.pow_dvd_pow_iff] at h_div_triv
...
theorem consecutive_square_ineq { x n : ℕ } (hn : x > n) : x^2 ≥ (n+1)^2 := by {
  exact Nat.pow_le_pow_of_le_left hn 2
}

theorem oktv_2022_ii_4 :
  IsGreatest { x : ℕ | IsGood x } max_x := by {
    ...
    · simp only [Set.mem_setOf_eq]
      ...
      have h₂ : 4^17 + 4^1020 + 4^x = 4^17 * (1 + 4^1003 + 4^(x-17)) := by {
          ...
          calc
            ...
            _ = 4^17 * 1 + 4^17 * 4^1003 + 4^17 * 4^(x-17) := by ring
      ...
      have expand_1_plus_2x_17_sq_old : (1 + 2^(x-17))^2 = 1 + 2^(x-16) + 4^(x-17) := by {
        calc
          _ = 1 + 2^(x-16) + 4^(x-17) := by linarith
      }

      have expand_1_plus_2x_17_sq : (2^(x-17) + 1)^2 = 1 + 2^(x-16) + 4^(x-17) := by {
        calc
          _ = 1 + 2^(x-16) + 4^(x-17) := by linarith
      }
      ...
      have h_x_leq_2022 : x ≤ 2022 := by {
        ...
        have : 1 + 2 ^ (x - 16) + 4^(x-17) ≤ 1 + 4 ^ 1003 + 4^(x-17) ↔ 2^(x-16) ≤ 4^1003 := by {
          constructor
          · intro h
            linarith
          · intro h
