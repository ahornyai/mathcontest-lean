...
theorem oktv_2024_i_4 :
  ∀ (a b c : ℕ), a > 0 ∧ b > 0 ∧ c > 0 ∧ 15 * (a * b * c + a + c) = 2024 * (b * c + 1) →
  (a, b, c) = (134, 1, 14) := by {
    intro a b c
    rintro ⟨apos, bpos, cpos, heq⟩
    ...
    have hbc_eq_14: b * c = 14 := by exact Eq.symm ((fun {a b} ↦ Nat.succ_inj'.mp) (id (Eq.symm hbc1_eq_15)))
